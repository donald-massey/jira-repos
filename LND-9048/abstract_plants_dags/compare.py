...
es23 = ds.Es23(dataset=DAG_DATASET, config=dag_config["elasticsearch"], source_connection_id=dag_config["denodo"]["connection_id"])
denodo = ds.Denodo(dataset=DAG_DATASET, config=dag_config["denodo"], source_connection_id=dag_config["source"]["connection_id"])
grs = ds.GeoRendering(config=dag_config["geo_rendering_service"], source_connection_id=dag_config["denodo"]["connection_id"])
...
def extend_es_load_dataset_meta(context, meta):
    ...
    ti.log.info(f"Got '{DENODO_LOADED_TABLES_XCOM_KEY}' from the '{LOAD_DENODO_TASK_ID}' task: " + ", ".join(loaded_source_tables))
...
def verify_new_counts(**context):
    ...
    source_count = get_nested_count(source_connection, 'pres.', source_tables[0], source_tables[1], source_tables[2], source_tables[3], "RecordId", "AbstractDocumentRecordId")
    ti.log.info(f'Source count: {source_count}')

    pres_connection = cfg['denodo']['connection_id']
    pres_views = cfg['denodo']['flip']['views']
    pres_count = get_nested_count(pres_connection, '', pres_views[0], pres_views[1], pres_views[2], pres_views[3], "RecordId", "AbstractDocumentRecordId")
    ...
    ti.log.info(f'Confirming source counts: (LowerBound) {low_count} <= (Actual) {source_count} <= (UpperBound) {high_count}')
...
def verify_load_counts(**context):
    ...
    abstract_document_grantor_grantee_table = ti.xcom_pull(key='denodo_load', task_ids='load_denodo_abstract_document_grantor_grantee')
    abstract_document_land_description_table_table = ti.xcom_pull(key='denodo_load', task_ids='load_denodo_abstract_document_land_description')
    abstract_document_prior_reference_table = ti.xcom_pull(key='denodo_load', task_ids='load_denodo_abstract_document_prior_reference')

    ti.log.info(f'Getting denodo count')
    denodo_count = get_nested_count(denodo_connection, '', primary_table, abstract_document_grantor_grantee_table, abstract_document_land_description_table_table, abstract_document_prior_reference_table, 'RecordId', 'AbstractDocumentRecordId')
...
PROD_VARIABLES = {
	...
	"source": {
		"connection_id": "ds9",
		"tables": ["abstractdocument", "abstractdocumentgrantorgrantee", "abstractdocumentlanddescription", "abstractdocumentpriorreference"],
		"control_table": {
	...
	"denodo": {
		"connection_id": "ds9",
		"flip": {
			"views": ["pres.vw_abstractdocument", "pres.vw_abstractdocumentgrantorgrantee", "pres.vw_abstractdocumentlanddescription", "pres.vw_abstractdocumentpriorreference"]
		}
	},
