...
# MAGIC %md
# MAGIC # Cibola (NM) OCR / IA-Summary publish-gap tracer
# MAGIC
# MAGIC Finds **where `OCRs3Path` drops out** in the courthouse-land-data-loader chain for
# MAGIC courthouse**Direct**Title (CHD) Cibola records, so we know how far up to roll back the
# MAGIC status dates.
# MAGIC
...
dbutils.widgets.text("county", "CIBOLA", "County name (contains, case-insensitive)")
dbutils.widgets.text("state", "NM", "State abbreviation")
...
# MAGIC %md ## 1. MANUFACTURE cache — is OCRs3Path populated for Cibola CHD records?
...
cibola = mfg.filter(F.upper(F.col(cn)).contains(COUNTY) & (F.upper(F.col(st)) == STATE))
total = cibola.count()
missing = cibola.filter(F.col(ocr).isNull()).count()
print(f"\nCibola manufactured records: {total}")
print(f"  with OCRs3Path:    {total - missing}")
print(f"  MISSING OCRs3Path: {missing}   <-- if ~3.6k, the OCR data never reached manufacture")

# COMMAND ----------

# Break the missing-OCR Cibola records down by source + modified month
if src:
    print("Missing-OCR Cibola records by source:")
    cibola.filter(F.col(ocr).isNull()).groupBy(src).count().show(truncate=False)

print("Missing-OCR Cibola records by _ModifiedDateTime month:")
(cibola.filter(F.col(ocr).isNull())
...
missing_ids = cibola.filter(F.col(ocr).isNull()).select(F.col(rid).alias("recordID"))
missing_ids.cache()
print(f"missing_ids count = {missing_ids.count()}")

# COMMAND ----------

# MAGIC %md ## 2. IMPORT OCR cache — did the June-27 OCR paths get imported at all?
# MAGIC The OCR paths come from CS_Digital (`enhancedclerkdatasource`) and join to CHD records by recordID.
# MAGIC If the missing recordIDs ARE here (with a June-27 date), manufacture just hasn't re-run.
...
print(f"\nOf {missing_ids.count()} Cibola records missing OCRs3Path in manufacture:")
print(f"  present in import/full OCR cache: {hit.count()}")
print("\n_ModifiedDateTime distribution of those matches (looking for 2026-06-27):")
...
# MAGIC - **Missing in manufacture, present in import OCR cache w/ June-27 dates:** re-run manufacture -> publish via the INCREMENT path.
# MAGIC - **Missing in import OCR cache too:** roll the IMPORT date back to before 2026-06-27, re-run acquire -> manufacture -> publish so the OCR increment is re-captured.
# MAGIC
