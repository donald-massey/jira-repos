"""query_3: verify which courthouseDirectTitle (CHD) Cibola recordIDs exist in
CS_Digital's COLE processing pipeline (cibola-nm_courthouseDirectTitle.csv).

CHD recordIDs are NOT present in CS_Digital.tblrecord (0/500 in testing) — they link
...
Usage (PowerShell):
  ...
  .\\.venv\\Scripts\\python.exe query_cs_digital_from_chd_csv.py --input cibola-nm_courthouseDirectTitle.csv --output cs_digital_match.csv --limit 500
"""
import argparse
import csv
import os
...
def main():
    ...
    parser.add_argument("--input", default="cibola-nm_courthouseDirectTitle.csv",
                        help="CHD export CSV (recordID in first column)")
    parser.add_argument("--output", default="cs_digital_match.csv", help="output CSV path")
