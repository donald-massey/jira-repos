/* ============================================================================
   {TICKET} — Level 2 follow-up: has COLE processed the image?
   Runbook: https://enverus.atlassian.net/wiki/spaces/DAQ/pages/38336692284

   Only relevant when query_2 shows the legal description itself is missing.
   Splits the outcome:
   ...
   COLE data model:
     ...
     DIML xref  {CSDIGITAL}.dbo.tblDimlXref              (recordID -> package_id)
     COLE log   {CSDIGITAL}.cole.tblRecordProcessingLogs (package_id -> OCR/IIE)

   Run on the CSTitle server (session is countyScansTitle-majority). Default
   {CSDIGITAL} = LINKTOPETL.CS_Digital (linked server); use bare CS_Digital only
   if the session already sits on the CS_Digital server.
   ============================================================================ */
...
FROM {CSDIGITAL}.[dbo].[tblDimlXref] x
...
FROM {CSDIGITAL}.[dbo].[tblDimlXref] x
JOIN {CSDIGITAL}.[cole].[tblRecordProcessingLogs] l ON l.package_id = x.package_id
JOIN @records rec ON rec.recordID = x.recordID;
...
FROM [countyScansTitle].[dbo].[tblS3Image] s
JOIN {CSDIGITAL}.[dbo].[tblDimlXref] x               ON x.recordID   = s.recordID
JOIN {CSDIGITAL}.[cole].[tblRecordProcessingLogs] l  ON l.package_id = x.package_id
JOIN @records rec ON rec.recordID = s.recordID;
