...
database_utils.py
=================
SQL Server connection wrapper for the LND-8093 S3Image backfill.
...
class DatabaseConnection:
    ...
    def execute_many(self, sql: str, params_seq: list[tuple], max_retries: int = 5) -> int:
        """Batch DML via cursor.executemany, returning rows affected; retries deadlocks."""
        if self._conn is None:
            raise RuntimeError(f"[{self.db_name}] Not connected. Call connect() first.")
        logger.info("[%s] EXECUTEMANY: %s | %d row(s)", self.db_name, sql.strip(), len(params_seq))
        ...
        for attempt in range(max_retries):
            try:
                ...
                if is_deadlock and attempt < max_retries - 1:
                    ...
                    logger.error("[%s] execute_many failed after %d attempt(s): %s", self.db_name, attempt + 1, e)
                    raise
